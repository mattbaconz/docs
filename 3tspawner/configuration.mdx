...
On first boot, 3TSpawner writes files under `plugins/3TSpawner/`. Most edits apply after `/3ts reload`. **Command root changes** (`commands.primary`, `aliases`, `gui-roots`) and toggling `shop.enabled` need a full server restart to register or unregister dedicated roots.
...
```yaml title="config.yml"
shop:
  enabled: true   # false: hide shop, block /3ts shop, skip /spawnershop
  auto-fill-mobs: true
...
Set `enabled: false` when spawners come from quests or crates. See [Shop](/3tspawner/shop).
