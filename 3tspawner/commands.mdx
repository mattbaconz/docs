...
| `/3ts shop` | `3tspawner.shop` | Open the spawner shop (`shop.enabled` must be true). |
...
| `/spawnershop` | Spawner shop (`shop.enabled` and a non-empty `gui-roots.shop` list) |

Set any root list to `[]` to disable that dedicated command. Empty `shop` roots stay empty (they are not restored to `/spawnershop`). `shop.enabled: false` also skips `/spawnershop` even if roots are listed.
...
| `/3ts give <player> <mob> [amount]` | `3tspawner.admin.give` | Give physical spawner items (quest/crate rewards). |
...
| `/3ts migrate …` | `3tspawner.admin.migrate` | Migration tools (**beta**). See [Migration](/3tspawner/migration). |
...
Migration subcommands: `scan`, `dryrun <json|csv|scan|smartspawner> [file]`, `apply <json|csv|scan|smartspawner> [file]`, `export`, `rollback`, `cancel`. Optional `--force` and `--owner <player>` on dryrun/apply.
